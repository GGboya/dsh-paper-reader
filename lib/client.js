...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let pendingJumpPage = 0
    function jumpToPaperPage(topic, name, page) {
    ...
    function ReaderOverlay() {
      ...
      useEffect(() => {
        const on = (e) => {
          // 跳转链接挂的页码（jumpToPaperPage 的跨论文路径）：随 src 传给阅读器 bootstrap
          const page = pendingJumpPage || undefined
          pendingJumpPage = 0
          setCur({ topic: e.detail.topic, name: e.detail.name, page })
        }
      ...
      if (!cur) return null
      const src = `/paper-reader/?topic=${encodeURIComponent(cur.topic)}&name=${encodeURIComponent(cur.name)}${cur.page ? '&page=' + cur.page : ''}`
      return createPortal(
        h('iframe', {
        ...
        try {
          ...
          if (!window.__dprJumpLinkBound) {
            ...
            document.addEventListener('click', (e) => {
              ...
              jumpToPaperPage(url.searchParams.get('topic'), url.searchParams.get('name'), page)
            }, true)
          }
