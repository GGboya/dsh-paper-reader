  ...
  factory: (require) => {
    ...
    function LibraryTree(props) {
      ...
      return h('div', { style: S.root, 'data-dsh-plugin': 'dsh-paper-reader', 'data-dsh-surface': 'sidebar' },
        // 常驻阅读器 iframe（覆盖在 PDF 页签的锚点上；不随会话重建）
        h(ReaderOverlay, null),
        // 对话列头部的历史/新建浮动按钮组（自渲染，不占树）
        h(SessionActions, { uiWorkspace }),
        h('input', { ref: fileInput, type: 'file', accept: '.pdf', style: { display: 'none' }, onChange: onFile }),
        // 阅读器和浮动按钮都不属于侧栏，收起侧栏时照常挂载；只换掉树本身
        tree,
      )
    }
      ...
      apply(ctx) {
        try {
          ...
          if (!document.getElementById('dpr-surface-css')) {
            ...
            styleEl.textContent = '[data-dpr-rail]:hover{background:var(--dsw-alias-interactive-bg-hover)!important}'
            document.head.appendChild(styleEl)
          }
          ...
          // （对话头部的历史/新建按钮不走 slot —— session 作用域 slot 不渲染动态包条目；
          //   SessionActions 由 LibraryTree 挂载，见下）

          // 侧栏中部：专题→论文→会话 树。
          // 槽传 { wide, expandSidebar }：收起成 rail 时 wide=false，注册方必须自己给一个
          // 图标入口（官方 WorkspaceBrowser 同款契约）——否则整棵树被塞进 56px 挤成竖排文字。
